...
-- 기본적으론 SUM 사용, MAX도 인정해주시기로 함 : )
SELECT CONCAT(MONTH(M.ORDER_DATE),'월') AS '월', P.NAME AS '상품명', SUM(COUNT) AS '판매수량'
FROM PRODUCT_INFO M
JOIN PRODUCT P ON M.ITEM_NO = P.NO
JOIN CUSTOMER C ON M.CUSTOMER_ID = C.ID
GROUP BY MONTH(M.ORDER_DATE), M.ITEM_NO
-- ORDER BY M.COUNT DESC -- MAX 쓸 경우
ORDER BY SUM(M.COUNT) DESC
LIMIT 3;
